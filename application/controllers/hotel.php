...
class Hotel extends CI_Controller {
	...
	public function index()
	{
		...
		if ($this->input->post('hotelId')) {
			$guestData = $this->input->post('guestData');
			$this->session->set_userdata(array(
				'hotelId' => $this->input->post('hotelId'),
				'hotelName' => $this->input->post('hotelName'),
				'arrival' => $this->input->post('arrival'),
				'departure' => $this->input->post('departure'),
				'guestData' => $guestData ? $guestData : '1,0,1'
			));
		...
		$data['title'] = ($hotelName ? $hotelName : 'Hotel') . " | MakeIFly";
		...
		if ($apiResponse && isset($apiResponse->Hotels->Hotel)) {
			...
			$SearchSessionId = isset($apiResponse->SearchSessionId) ? $apiResponse->SearchSessionId : null;
			$hotelCurrency = isset($apiResponse->Currency) ? $apiResponse->Currency : '&#8358;';
		} else {
			// Log for debugging
		...
		$defaults->Booking->DepartureDate = $afterOneMonth;
		$defaults->Booking->CountryCode = $countryCode;
		$defaults->Booking->City = $cityCode;
		$defaults->hotelName = $hotelName;
		$defaults->currency = $hotelCurrency;
		$defaults->guests = $guests;
		$defaults->arrival = $arrival;
		$defaults->departure = $departure;
		$defaults->rooms = $rooms;
		$defaults->adult = $adults;
	...
	public function search()
	{
		...
		if ($apiResponse) {
			...
			$data['hotels'] = isset($apiResponse->Hotels->Hotel) ? $apiResponse->Hotels->Hotel : array();
		}
